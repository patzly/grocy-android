...
public class FilterChipLiveDataStockEntriesSort extends FilterChipLiveData {
  ...
  public FilterChipLiveDataStockEntriesSort(Application application, Runnable clickListener) {
    this.application = application;
    setItemIdChecked(-1);

    sharedPrefs = PreferenceManager.getDefaultSharedPreferences(application);
    sortMode = sharedPrefs.getString(PREF.STOCK_ENTRIES_SORT_MODE, SORT_NAME);
    sortAscending = sharedPrefs.getBoolean(PREF.STOCK_ENTRIES_SORT_ASCENDING, true);
    setFilterText();
    setItems();
  ...
  public void setValues(int id) {
    if (id == ID_SORT_NAME) {
      sortMode = SORT_NAME;
      setFilterText();
      sharedPrefs.edit().putString(PREF.STOCK_ENTRIES_SORT_MODE, sortMode).apply();
      ...
      sharedPrefs.edit().putBoolean(PREF.STOCK_ENTRIES_SORT_ASCENDING, sortAscending).apply();
    }
  }
