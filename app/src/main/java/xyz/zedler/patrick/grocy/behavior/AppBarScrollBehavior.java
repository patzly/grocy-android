...
public class AppBarScrollBehavior {
  ...
  public void setUpScroll(
    ...
    if (DEBUG) {
      Log.i(TAG, "setUpScroll: liftOnScroll = " + liftOnScroll);
    }
  }
  ...
  /**
   * Sets the global boolean and moves the appBar manually if necessary. If scrollY of the
   * scrollView is 0, OverScroll is turned off. Otherwise it's on if the the view is scrollable.
   */
  public void setLiftOnScroll(boolean lift) {
    ...
    if (appBarLayout != null) {
      ...
      Log.e(TAG, "setLiftOnScroll: appBarLayout is null!");
    }
    if (DEBUG) {
  ...
  /**
   * Tints the navBarDivider with divider color if setUpBottomAppBar wasn't called before and the
   * scrollView is scrollable, else transparent.
   */
  private void setNavBarDividerVisibility() {
    if (activity != null) {
      ...
      if (orientation == Configuration.ORIENTATION_PORTRAIT) {
        ...
        if (DEBUG) {
          Log.i(TAG, "setNavBarDividerVisibility(" + showNavBarDivider + ")");
        }
      } else {
        setNavBarDividerColor(R.color.stroke_secondary);
      }
    } else if (DEBUG) {
      Log.wtf(TAG, "setNavBarDividerVisibility: activity is null!?");
    }
  }
  ...
  private void tintTopBars(@ColorRes int target) {
    if (activity != null && linearAppBar != null) {
      ...
      if (appBarColor != targetColor || statusBarColor != targetColor) {
        ...
        if (DEBUG) {
          Log.i(TAG, "tintTopBars: appBarLinearLayout and status bar tinted");
        }
      } else {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M
            && appBarColor == ContextCompat.getColor(activity, R.color.white)
      ...
      Log.e(TAG, "tintTopBars: activity or appBarLinearLayout is null!");
    }
  }
}
