...
*In the v2.0.0 alpha preview version of our app you can login with a long-term token from Home Assistant (together with a grocy API key). You won't need to open a port then anymore.
  To set it up, enter your IP address with the according port (e. g. 8123). Get a long lived access token from your Home Assistant and an API key from your Grocy instance. To get the ingress instance name, go into Grocy -> Settings -> REST-API browser. In the window which opened, select the part of the URL between /hassio_ingress/ and /api/. That´s your ingress instance.*
